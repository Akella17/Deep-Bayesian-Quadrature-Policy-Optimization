...
def pca_Cov(args, conjugate_gradients, Fvp, policy_net, value_net, likelihood):
    ...
    with gpytorch.settings.max_cg_iterations(1000), gpytorch.settings.max_preconditioner_size(50), gpytorch.settings.fast_pred_var():
        ...
        values_mvn = value_net(args.GP_inputs, state_multiplier = args.state_coefficient, fisher_multiplier = fisher_multiplier)
...
def pca_Cov_Exact(args, conjugate_gradients, Fvp, policy_net, value_net, likelihood):
...
def pca_InvCov(args, conjugate_gradients, Fvp, policy_net, value_net, likelihood):
    ...
    for row_vectors in R:
        ...
        with gpytorch.settings.max_cg_iterations(1000), gpytorch.settings.max_preconditioner_size(50), gpytorch.settings.fast_pred_var():
            fisher_multiplier = args.fisher_coefficient*args.v_tens.shape[0]
            values_mvn = value_net(args.GP_inputs, state_multiplier = args.state_coefficient, fisher_multiplier = None, only_state_kernel = True)
            second_term_1 = likelihood(values_mvn).lazy_covariance_matrix.inv_matmul(second_term_0)
        grads = torch.autograd.grad((U_prob*Variable(second_term_1.detach())).sum(0), policy_net.parameters(), retain_graph = True)
        second_term = torch.cat([grad.view(-1) for grad in grads])
...
def pca_NPG_InvCov(args, conjugate_gradients, Fvp, policy_net, value_net, likelihood):
